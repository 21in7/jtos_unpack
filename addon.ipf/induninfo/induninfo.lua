...
function INDUNINFO_UI_OPEN(frame, index, selectIndun)
    ...
    local cyclePic = GET_CHILD_RECURSIVELY(frame, 'cyclePic')
    local pvpShopBtn = GET_CHILD_RECURSIVELY(frame, 'pvpShopBtn')
    local pvpCharacterRegistBtn = GET_CHILD_RECURSIVELY(frame, 'pvpCharacterRegistBtn')
    
    if config.GetServiceNation() == "GLOBAL_JP" then
        if cyclePic ~= nil then
            cyclePic:Resize(48, 27)
        end

        if pvpShopBtn ~= nil then
            pvpShopBtn:SetTextTooltip('{@st66b}{s16}{#FFFFFFF}チームバトルリーグ商店')
        end

        if pvpCharacterRegistBtn ~= nil then
            pvpCharacterRegistBtn:SetTextTooltip('{@st66b}{s16}{#FFFFFFF}交代キャラクター登録')
        end
    elseif config.GetServiceNation() == "GLOBAL" then
        if cyclePic ~= nil then
            cyclePic:Resize(48, 27)
            cyclePic:SetMargin(140, 2, 0, 0)
        end
...
function TOGGLE_INDUNINFO(frame,type)
	...
	do
        ...
        local joinpracticeenter = GET_CHILD_RECURSIVELY(frame, 'joinpracticeenter');
        local battle_info_time_text = GET_CHILD_RECURSIVELY(frame, 'battle_info_time_text');
        local gauge = GET_CHILD_RECURSIVELY(frame, 'gauge')
        local joinenter = GET_CHILD_RECURSIVELY(frame, 'joinenter')
        if isShow == 1 then
            battle_info_time_text:ShowWindow(1)
            gauge:ShowWindow(1)
            joinpracticeenter:ShowWindow(1)
            joinenter:ShowWindow(1)
        end
	end
	--field boss
	do
...
function IS_WEEKENDEVENT_CONTENTS(name)
    if config.GetServiceNation() == 'PAPAYA' then -- #124829
        return false
    end
...
function INDUNINFO_DRAW_CATEGORY_DETAIL_LIST(indunListBox, cls, is_weekly_reset, selectIndun)
    ...
    local _level = TryGetProp(cls, 'PresentLevel', 0);
    if _level == 0 then    
        _level = TryGetProp(cls, 'Level', 0);
    end

    infoText:SetTextByKey('level', _level);
    ...
    if TryGetProp(cls,"Difficulty","None") == "None" then        
        ...
        nameText:SetTextTooltip(prefix .. cls.Difficulty)
        ...
        if TryGetProp(cls, 'TicketingType', 'None') == 'Entrance_Ticket' then        
            ...
            if string.find(name, 'Challenge_Auto_Hard_Party') == nil and string.find(name, 'Challenge_Normal_Solo') == nil then
            cyclePic:ShowWindow(0);
        else
            ...
            if name == "Challenge_Normal_Solo" then
                cyclePic:ShowWindow(0);
            else
                INDUNINFO_SET_CYCLE_PIC(cyclePic,cls,'_s')
            end
    ...
    if table.find(g_contentsCategoryList,selectedGroupID) ~= 0  then
        ...
        for i = 0, cnt - 1 do
            ...
            if add_flag == true then
				...
				if indunCls.DungeonType == "Solo_dungeon" or indunCls.DungeonType == "MythicDungeon_Auto_Hard" or indunCls.DungeonType == "MythicDungeon" or indunCls.DungeonType == "TOSHero" or indunCls.DungeonType == "BridgeWailing" then
...
function GET_RESET_CYCLE_PIC_TYPE(cls,postFix)
    ...
    elseif idSpace == "Indun" then
        ...
        local indun_class_name = TryGetProp(indunCls, "ClassName", "None")
...
function INDUNINFO_MAKE_DETAIL_INFO_BOX(frame, indunClassID)
    ...
    INDUNINFO_SET_ENTERANCE_COUNT(frame,resetGroupID, indunClassID)
...
function INDUNINFO_MAKE_DETAIL_COMMON_INFO(frame, indunCls, resetGroupID)
    ...
    if TryGetProp(indunCls, 'TicketingType', 'None') == 'Entrance_Ticket' then
        local cyclePic = GET_CHILD_RECURSIVELY(frame, 'cyclePic')
        cyclePic:ShowWindow(0)
    end
...
function INDUNINFO_SET_ENTERANCE_TIME(frame,indunCls)
    ...
    if GetServerNation() == "GLOBAL" then
        ...
        if GetServerNation() == "PAPAYA" then
            local ruleCls = GetClass("pvp_teambattle_rule", "pvp_teambattle_rule_PAPAYA")
            if ruleCls ~= nil then
                indunCls = ruleCls
            end
        end
...
function INDUNINFO_SET_ENTERANCE_COUNT(frame, resetGroupID, indunClassID)
    --todo
    local count_box = GET_CHILD_RECURSIVELY(frame, "countBox");
    count_box:ShowWindow(1);
    local dungeon_restrict_category_box = GET_CHILD_RECURSIVELY(frame, "gbox_ct_dungeon_restrict")
    dungeon_restrict_category_box:ShowWindow(0);
